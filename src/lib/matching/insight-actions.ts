...
export type InsightRunResult =
  | {
      ok: true;
      /** Offers judged this run (0 = everything was already fresh). */
      analyzed: number;
      /** Offers whose individual analysis failed (logged; the rest landed). */
      failed: number;
    }
  | { ok: false; error: "unconfigured" | "no_profile" | "generic" };
...
export async function explainMatchesAction(): Promise<InsightRunResult> {
  try {
    ...
    if (targets.length === 0) return { ok: true, analyzed: 0, failed: 0 };
